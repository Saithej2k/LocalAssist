...
| --- | --- |
| On-device structured summaries and task suggestions | `LocalAssistFoundationModels` calls `SystemLanguageModel.default.availability` and streams `LanguageModelSession.streamResponse(to:)`; `LocalAssistCore` validates the guided JSON into typed `StructuredSummary` and `TaskSuggestion` models. |
...
| Streaming and cancellation | `LanguageModelClient.streamResponse(for:)` returns an `AsyncThrowingStream`; `LocalAssistService.streamSummary(_:)` emits phases and partial guided JSON, validates only the final response, and cancellation is covered by XCTest and self-tests. |
| System-level integration | `LocalAssistSummaryIntent`, `CreateReminderDraftIntent`, and `ShowRecentRunsIntent` expose the workflow through App Intents, and `LocalAssistShortcuts` publishes Shortcut phrases. |
...
| Automated tests | `LocalAssistCoreTests` and `localassist-selftest` cover malformed input, model availability, malformed model output, streaming partials, concurrent requests, cancellation, offline execution, deterministic fallback, run metrics, action preparation, metric distributions, and history persistence. |
| iOS app surface | `project.yml` generates `LocalAssist.xcodeproj`; `LocalAssistAppUI` contains the SwiftUI app experience and `Apps/iOS/LocalAssist` contains the app entry point and iOS metadata. |
| iOS screenshots | `docs/screenshots` contains deterministic mockups and real iOS 26.5 simulator captures from the `LocalAssist` app target. |
